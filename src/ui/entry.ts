...
export function createEntryScreen(opts: {
  ...
  // Source link — bottom-right. Inline SVG (GitHub's own mark, MIT-licensed
  // Octicons) so there's no network fetch and no asset to credit.
  const ghLink = document.createElement('a');
  ghLink.href = 'https://github.com/beanthemoonman/dougiesbigtrip';
  ghLink.target = '_blank';
  ghLink.rel = 'noopener noreferrer';
  ghLink.title = 'Source on GitHub';
  ghLink.style.cssText =
    `position:absolute;bottom:16px;right:16px;display:flex;align-items:center;gap:8px;` +
    `color:${MUTED};text-decoration:none;font:12px monospace;opacity:0.6;`;
  ghLink.innerHTML =
    ...
    '<span>Source</span>';
  ghLink.onmouseenter = (): void => { ghLink.style.opacity = '1'; };
  ghLink.onmouseleave = (): void => { ghLink.style.opacity = '0.6'; };
