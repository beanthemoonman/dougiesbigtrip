import { Box3, CanvasTexture, Color, DoubleSide, FogExp2, Group, MathUtils, Mesh, MeshBasicMaterial, PlaneGeometry, SkinnedMesh, type MeshStandardMaterial, Object3D, Quaternion, SRGBColorSpace, Vector3 } from 'three';
...
import { type Breakable, damageProp, resetBrokenBreakables } from './game/breakables';
...
const PROP_PLACEMENTS: readonly [string, number, number, number, number?, number?][] = [
...
function makeSign(label: string, arrowDir: 'left' | 'right' | 'up' | 'down'): Group {
  const size = 256;
 ...
 */
async function placeProps(scene: Object3D, world: import('@dimforge/rapier3d-compat').World): Promise<{ placed: PlacedProp[]; templates: Map<string, PropTemplate> }> {
  ...
  for (const [url, x, z, yaw, stack = 0, tintHex] of PROP_PLACEMENTS) {
...
async function main(): Promise<void> {
  ...
  function restoreBreakables(): void {
    ...
    for (const i of reset) {
      const [url, x, z, yaw, stack = 0, tintHex] = PROP_PLACEMENTS[i]!;
      const model = propTemplates.get(url);
      if (!model) continue;
      const { root, box } = model;
      box.getSize(size);
