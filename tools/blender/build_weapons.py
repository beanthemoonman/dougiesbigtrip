...
def mat(name, color, metallic, roughness, noise_seed=0, noise_scale=0.06, mix_factor=0.08):
    # ponytail: base colour is a *plain* value — earlier this routed the colour
    # through an RGB→Mix(noise)→Base Color graph for surface breakup, but the
    # glTF exporter can't represent that graph: it dropped the colour (exported
    # baseColorFactor=white + the grey noise as the baseColorTexture), so every
    # weapon rendered as flat grey noise. noise_* args kept for call-site compat.
    # Want breakup back? Bake it into a proper texture, don't drive Base Color
    # with a node graph.
...
def build_ak(M):
    ...
    parts.extend(swept_part("AK_grip", os.path.join(CURVES, "ak_grip.curve.json"),
                            (0, -0.01, -0.035), 0.032, 0.052, 0.15, M["wood"], head=1, bevel=0.010))
